---
source: /Users/byron/projects/rockbeatspaper/whatever.py
relative: rockbeatspaper/whatever.py
...
from thrml.models.discrete_ebm import CategoricalEBM, CategoricalSamplingProgram
from thrml.sampling_programs import FactorSamplingProgram
...
benford_probs = jnp.log(1 + 1 / jnp.arange(1, 10)) # Use JAX for consistency

# Observed frequencies (proportions)
obs_counts, _ = np.histogram(observed, bins=range(10), range=(1, 10))
...
sim_counts, _ = np.histogram(sim_samples_np + 1, bins=range(10), range=(1, 10)) # Back to 1-9
