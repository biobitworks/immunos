---
source: /Users/byron/projects/scripts/immunos_universal.py
relative: scripts/immunos_universal.py
...
Integrated with IMMUNOS database at .immunos/db/immunos.db

Based on:
- Umair et al. (2025) - Negative Selection AIS
- Opus 4.5 Universal Framework Research
"""
...
class ImmuneAlgorithm(ABC):
    ...
    @abstractmethod
    def train(self, self_samples: np.ndarray, config) -> List[Detector]:
...
class NegativeSelection(ImmuneAlgorithm):
    ...
    def train(self, self_samples: np.ndarray, config) -> List[Detector]:
        ...
        while len(detectors) < config.num_detectors and attempts < max_attempts:
            ...
            if self._passes_selection(candidate, self_normalized, config.r_self):
                ...
                detector = Detector(
                    id=f"nsa_{config.name}_{len(detectors):04d}",
    ...
    def _calculate_radius(self, candidate: np.ndarray,
        ...
        return max(0.0, min_distance - r_self)
...
class ImmuneMemory:
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            # Use IMMUNOS default database
            project_root = Path(__file__).parent.parent
            db_path = str(project_root / ".immunos" / "db" / "immunos.db")

        self.db_path = db_path
        self._init_db()

    def _init_db(self):
        """Initialize immune_memory table if not exists"""
        ...
        if not cursor.fetchone():
            ...
            cursor.execute('''
                ...
                ON immune_memory(domain, pattern_hash)
            ''')
    ...
    def store(self, domain: str, pattern: np.ndarray,
        ...
        cursor.execute('''
            SELECT id, hit_count FROM immune_memory
...
class ImmuneSystem:
    """
    Main IMMUNOS Universal System

    Combines all AIS algorithms into unified framework for 5 domains:
    - Emotion Recognition
    - LLM Hallucination Detection
    - Network Intrusion Detection
    - Code Vulnerability Detection
...
if __name__ == "__main__":
    ...
    print("IMMUNOS Universal Framework Test")
    print("=" * 60)

    # Import configuration
    try:
        from immunos_config import DEFAULT_CONFIGS, DomainConfig
    except ImportError:
    ...
    print(f"✓ Statistics: {stats['num_detectors']} detectors, "
          f"{stats['total_matches']} total matches")

    print("\n" + "=" * 60)
    print("IMMUNOS Universal Framework Ready!")
    print("5 domains supported: emotion, hallucination, network, code, research")
