---
source: /Users/byron/projects/immunos-mcp/src/immunos_mcp/algorithms/negsel.py
relative: immunos-mcp/src/immunos_mcp/algorithms/negsel.py
...
try:
    from immunos_mcp.config.paths import get_data_root, resolve_data_path
except ImportError:
    def get_data_root(override: Optional[str] = None) -> Path:
        value = override or os.getenv("IMMUNOS_DATA_ROOT")
        return Path(value) if value else Path("/Users/byron/projects/data/immunos_data")
...
def train_safe_code_detector(safe_code_dir: str, num_detectors: int = 20,
                             r_self: float = 0.85,
                             db_path: Optional[str] = None) -> NegativeSelectionClassifier:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('safe_code_dir', nargs='?',
                        help='Directory containing safe code samples '
                             '(absolute or relative to data root)')
    parser.add_argument('--data-root', default=None,
                        help='Dataset root (uses IMMUNOS_DATA_ROOT if set)')
    ...
    parser.add_argument('--db-path', default=None,
                        help='SQLite DB path for detector persistence')
    ...
    if args.command == 'info':
        ...
        if not args.safe_code_dir:
            ...
            print("Tip: set IMMUNOS_DATA_ROOT and pass a relative path, e.g.")
            print("  IMMUNOS_DATA_ROOT=/Users/byron/projects/data/immunos_data")
            print("  python immunos_negsel.py train code/safe_samples")
            sys.exit(1)

        safe_code_dir = str(resolve_data_path(args.safe_code_dir, args.data_root))
        print("IMMUNOS Detector Training")
        print("=" * 50)
        print(f"Safe code directory: {safe_code_dir}")
        print(f"Number of detectors: {args.num_detectors}")
        print(f"R_self threshold: {args.r_self}")
        print()

        classifier = train_safe_code_detector(
            safe_code_dir=safe_code_dir,
