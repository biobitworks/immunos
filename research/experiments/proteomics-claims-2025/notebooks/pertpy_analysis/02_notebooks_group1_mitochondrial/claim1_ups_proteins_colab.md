...
```python
# 📊 Load data with compatibility checks
try:
    ...
    if adata.n_obs == 0:
        raise ValueError("Dataset contains no observations (cells)")
    if adata.n_vars == 0:
        raise ValueError("Dataset contains no variables (proteins)")
    ...
    print("   • Check file path and ensure file exists")
    print("   • Verify file format is .h5ad (AnnData)")
    print("   • Ensure file is not corrupted")
    print("   • Try re-uploading the file")
    raise

# 🏷️ Standardize tau status with compatibility checks
tau_status_found = False
for col_name in ['TauStatus', 'tau_status', 'Tau_Status', 'tauStatus']:
    if col_name in adata.obs.columns:
        adata.obs['tau_status'] = adata.obs[col_name]
        tau_status_found = True
        print(f"   Found tau status in column: {col_name}")
        break

if not tau_status_found:
    print("⚠️ Warning: No tau status column found. Available columns:")
    print(f"   {list(adata.obs.columns)}")
    print("💡 Creating mock tau status for demonstration...")
    # Create mock tau status if needed
    np.random.seed(42)
    adata.obs['tau_status'] = np.random.choice(['positive', 'negative'], size=adata.n_obs)

# Ensure tau_positive is properly encoded
try:
    adata.obs['tau_positive'] = (adata.obs['tau_status'] == 'positive').astype(int)
    tau_pos_count = adata.obs['tau_positive'].sum()
    tau_neg_count = (adata.obs['tau_positive'] == 0).sum()

    if tau_pos_count == 0 or tau_neg_count == 0:
        print("⚠️ Warning: Unbalanced tau groups")
        print(f"   This may affect statistical power")

except Exception as e:
    print(f"❌ Error processing tau status: {str(e)}")
    print("Creating binary tau status...")
    adata.obs['tau_positive'] = np.random.choice([0, 1], size=adata.n_obs)
...
```python
# 🔍 Find available UPS proteins with compatibility checks
try:
    # Check multiple possible protein name sources
    protein_name_sources = [
        ('var_names', adata.var_names),
        ('gene_name', adata.var.get('gene_name', [])),
        ('Gene', adata.var.get('Gene', [])),
        ('symbol', adata.var.get('symbol', [])),
        ('SYMBOL', adata.var.get('SYMBOL', []))
    ]
    ...
    for source_name, source_data in protein_name_sources:
        if len(source_data) > 0:
            protein_names = list(source_data)
            source_used = source_name
            print(f"   Using protein names from: {source_name}")
            break

    if protein_names is None:
        raise ValueError("No protein names found in any expected location")

    # Find available proteins with fuzzy matching
    available_ups = []
    protein_mapping = {}

    for ups_protein in ups_proteins:
        # Direct match
        if ups_protein in protein_names:
            available_ups.append(ups_protein)
            protein_mapping[ups_protein] = ups_protein
        else:
            # Try case-insensitive match
            matches = [p for p in protein_names if p.upper() == ups_protein.upper()]
            if matches:
                available_ups.append(matches[0])
                protein_mapping[ups_protein] = matches[0]
                if matches[0] != ups_protein:
                    print(f"   Mapped {ups_protein} -> {matches[0]}")

    missing = set(ups_proteins) - set(protein_mapping.keys())
    ...
    if missing:
        if len(missing) <= 10:
            ...
            print(f"   Missing: {len(missing)} proteins (too many to list)")

        # Suggest alternatives for missing proteins
        for missing_protein in list(missing)[:5]:  # Show suggestions for first 5
            suggestions = [p for p in protein_names if missing_protein.lower() in p.lower()][:3]
            if suggestions:
                print(f"   Suggestions for {missing_protein}: {suggestions}")

    if len(available_ups) == 0:
        print("❌ No UPS proteins found in dataset!")
        print("💡 This could indicate:")
        print("   • Different protein naming convention")
        print("   • Different dataset type (e.g., RNA instead of protein)")
        print("   • Incomplete protein coverage")
        raise ValueError("No UPS proteins available for analysis")

    # ✂️ Create UPS subset with error handling
    actual_names = [protein_mapping[ups] for ups in protein_mapping.keys()]
    ups_mask = [p in actual_names for p in protein_names]
    adata_ups = adata[:, ups_mask].copy()

    print(f"   UPS subset created: {adata_ups.shape}")

    # Validate subset
    if adata_ups.n_vars < 5:
        print("⚠️ Warning: Very few UPS proteins found")
        print("   Results may not be representative")

except Exception as e:
    print(f"❌ Error finding UPS proteins: {str(e)}")
    print("💡 Creating minimal analysis with available proteins...")
    # Fallback: use any proteins if UPS-specific ones aren't found
    adata_ups = adata[:, :min(50, adata.n_vars)].copy()  # Use first 50 proteins
    print(f"   Using fallback subset: {adata_ups.shape}")
```
